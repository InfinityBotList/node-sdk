...
class Console {
    ...
    _buildConsole(type, ...args) {
        ...
        const console = this._meta()

        if (additional.prefix) {
            if (this._config.underlinePrefix) {
                console.push(underline(additional.prefix))
            } else {
                console.push(additional.prefix)
            }
        }

        if (this._config.displayBadge && type.badge) {
            console.push(chalk[type.color](this._padEnd(type.badge, type.badge.length + 1)))
        }
        ...
        if (this._config.displayLabel && type.label) {
            ...
            if (this._config.underlineLabel) {
                console.push(chalk[type.color](this._padEnd(underline(label), this._longestUnderlinedLabel.length + 1)))
            } else {
                console.push(chalk[type.color](this._padEnd(label, this._longestLabel.length + 1)))
            }
        }
        ...
        if (msg instanceof Error && msg.stack) {
            ...
            if (this._config.underlineMessage) {
                console.push(underline(name))
            } else {
                console.push(name)
            }

            console.push(grey(rest.map(l => l.replace(/^/, '\n')).join('')))
            return console.join(' ')
        }

        if (this._config.underlineMessage) {
            console.push(underline(msg))
        } else {
            console.push(msg)
        }
        ...
        if (additional.suffix) {
            if (this._config.underlineSuffix) {
                console.push(underline(additional.suffix))
            } else {
                console.push(additional.suffix)
            }
        }
    ...
    _logger(type, ...messageObj) {
        ...
        const message = this._buildConsole(this._types[type], ...messageObj)
    ...
    scope(...name) {
        ...
        return new Console(Object.assign(this.currentOptions, { scope: name }))
    }
